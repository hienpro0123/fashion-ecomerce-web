...
- Main app data uses **Cloud Firestore**, not Realtime Database.
- Customer cancel order uses **status update**, not delete.
- Build script currently uses `cp`, which may fail on Windows after Vite finishes building.
- Wish list and live payments are not fully implemented yet.

---

## Why This Project Is Worth Reviewing

- Real ecommerce flow from catalogue to checkout to order tracking
- Role-based admin/customer experience
- Firebase-integrated frontend architecture
- Redux Saga side-effect handling
- AI assistant grounded in live product data
